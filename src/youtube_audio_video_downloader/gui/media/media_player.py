...
from youtube_audio_video_downloader.gui.components.widgets import GlassCard
from youtube_audio_video_downloader.gui.media.audio_visualizer import SpectrumAnalyzer
from youtube_audio_video_downloader.gui.runtime.crash_reporter import log_diagnostic
from youtube_audio_video_downloader.gui.components.resources import asset_path
...
class MediaLibraryPage(QWidget):
    ...
    def _load_current(self, *, preserve_video_display_modes: bool = False) -> None:
        if not 0 <= self.queue_index < len(self.queue):
            return
    ...
    def play(self) -> None:
        ...
        if self.player.source().isEmpty() and self.queue:
            ...
            if (
                self._reset_video_display_on_play
                and not self._remember_video_display_modes
                and 0 <= self.queue_index < len(self.queue)
                and self.queue[self.queue_index].media_type == MEDIA_TYPE_VIDEO
            ):
                self._video_aspect_index = 0
                self._video_crop_index = 0
                self._apply_video_display_modes()
    ...
    def stop(self) -> None:
        ...
        self._reset_video_display_on_play = (
            0 <= self.queue_index < len(self.queue)
            and self.queue[self.queue_index].media_type == MEDIA_TYPE_VIDEO
        )
        self.player.stop()
    ...
    def _advance_after_end(self) -> None:
        ...
        if self._repeat_mode == "one":
            ...
            preserve_display_modes = len(self.queue) == 1 and self.queue_index == 0
            self.queue_index = 0
            self._load_current(
                preserve_video_display_modes=preserve_display_modes
            )
        else:
            self.stop()
            self._update_queue_status()
