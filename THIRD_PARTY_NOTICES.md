...
- **FFmpeg and FFprobe** are invoked as separate processes. Windows x64 packages use the checksum-pinned 8.1.1 Essentials build from Gyan Doshi, macOS packages use the 7.1 build obtained by `portable-ffmpeg` from OSXExperts, and Linux x64 packages use the checksum-pinned, self-contained 6.0.1 static build from John Van Sickle. The selected builds are distributed under GNU GPL v3; FFmpeg source and licensing information are available from <https://ffmpeg.org/>, and the Windows build source is available from <https://github.com/GyanD/codexffmpeg>.
